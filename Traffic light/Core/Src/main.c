...
void Timer1_Interrupt()
{
	if (timerCount % 16000 == 0)
	{
		sFlags[7] = 1;
		timerCount = 0;
	}
	else if (timerCount % 14000 == 0)
	{
		sFlags[6] = 1;
	}
	else if (timerCount % 11000 == 0)
	{
		sFlags[5] = 1;
	}
	else if (timerCount % 9000 == 0)
	{
		sFlags[4] = 1;
	}
	else if (timerCount % 8000 == 0)
	{
		sFlags[3] = 1;
	}
	else if (timerCount % 6000 == 0)
	{
		sFlags[2] = 1;
	}
	else if (timerCount % 3000 == 0)
	{
		sFlags[1] = 1;
	}
	else if(timerCount % 1000 == 0)
	{
		sFlags[0] = 1;
	}
	++timerCount;
...
void Blink(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin)
{
	...
	{
		now = HAL_GetTick();
			if (now - then >= 100) { // Only if the current tick is 500 ms after the last

				HAL_GPIO_TogglePin(GPIOx, GPIO_Pin); // Toggle LED
